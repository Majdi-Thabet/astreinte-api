...
//
    ...
    public ResponseEntity<Collaborateur> updateCollaborateur(@PathVariable("id") long id, @RequestBody Collaborateur collaborateur) {
        Optional<Collaborateur> collaborateurData = collaborateurRepository.findById(id);
...
//        @PostAuthorize("hasAuthority('ADMIN')")
    public Collaborateur saveUser(@RequestBody Collaborateur collaborateur, @PathVariable Long idRole, @PathVariable Long iddepartment) throws UserAlreadyExistException {
        ...
        if (authToken != null && authToken.startsWith(JWTUtil.PREFIX)) {
            try {
                ...
                String jwtAccessToken = JWT.create()
                        ...
                        .withClaim("roles", collaborateur.getRoles().stream().map(r -> r.getRoleName()).collect(Collectors.toList()))
                        .sign(algorithm);
                ...
                //intercepter la requete faire les verifications puis je passe au filtre suivant wou mba3ed requete temchi lil dipatcher servlets
                //authentifier l'user
